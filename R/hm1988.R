...
#" @export
hm1988 <- function(env, formula, data, choicerule, fix = list(), options = NULL) {
  .args <- as.list(rlang::call_standardise(match.call())[-1])
  return(do.call(Hm1988$new, .args))
}
...
Hm1988 <- R6Class("hm1988",
  ...
  public = list(
    ...
    env = "rsenvironment",
    choicerule = NULL,
    initialize = function(env = NA, data, formula, choicerule, fix = list(), options = list()) {
      if (missing(formula)) {
      ...
      predict = function(type = c("response", "values", "ev", "pstates"), action = NULL, newdata = NULL) {
        ...
        if (type == "response" | type == "values") {
          ...
          if (type == "values") {
            ...
            return(super$apply_choicerule(v)[, action])
          }
        } else if (type == "ev") {
          return(self$EV[cbind(rows, cols, 1)])
        } else if (type == "pstates") {
          ps <- self$V
          ps[] <- super$apply_choicerule(matrix(ps, nc = self$env$n.actions))
      ...
      makeEVMat = function() {
        ...
        P <- t(apply(self$V, 1, super$apply_choicerule))
