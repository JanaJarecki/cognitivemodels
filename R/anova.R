...
anova.cogscimodel <- function(object, ...) {
  ...
  if (any(named)) {
    warning("These arguments to 'anova.cogscimodel' are invalid and dropped: ",
  ...
  is.allowed.class <- vapply(dotargs, function(x) inherits(x, "cogscimodel"), NA) | vapply(dotargs, function(x) inherits(x, "lm"), NA) | vapply(dotargs, function(x) inherits(x, "merMod"), NA)
...
anova.cogscimodellist <- function(models, model.names = NULL) {
  ...
  is.cogscimodel <- vapply(models, function(x) inherits(x, "cogscimodel"), NA)
  ...
  if (any(substr(modelnames, 1,4) == "new(") ||
      any(duplicated(modelnames[!is.cogscimodel])) || ## <- only if S4 objects are *not* properly deparsed
...
print.anova.cogscimodel <- function(x, digits = max(getOption("digits") - 4L, 2L), ...) {
