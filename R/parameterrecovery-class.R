...
#' No examples yet
#' @export
parameterrecovery <- function(model, nruns = 100, nsteps = list(), seed = 98634) {
  Parameterrecovery$new(model=model, nruns=nruns, nsteps=nsteps, seed=seed)

}

Parameterrecovery <- R6Class(
  'parameterecovery',
  public = list(
    model = NULL,
    parameter = NULL,
    ...
    true = NULL,
    recovered = NULL,
    initialize = function(model, nruns, nsteps, seed) {
    ...
    long = function() {
      melt(self$recovered, 1:2, value = 'recovered')[melt(self$true, 1, value = 'true'), on = c('id', 'variable')]
    },
