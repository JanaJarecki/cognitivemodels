...
test_that('Predicted Values for 1 row, 2 alternatives', {
  ...
  m1 <- bayes(~ x + y, data = D[1,],  fix = fp[1:3], mode = "c")
  m2 <- bayes(~ x + y, D[1,], fix=list(delta=1, priorpar=c(1,1)), mode = "c")
  m3 <- bayes_beta_c(~ x + y, D[1,], fix = fp[1:3])
  mc2 <- bayes_beta_c(~ x + y, data = DC[1,], fix = fp[1:3], format = "cumulative")
...
test_that('Predicted Values for 2 alternatives', {
  ...
  m3 <- bayes_beta_c(~ x + y, D, fix = fp[1:3])
  mc <- bayes(~ x + y, data = DC, fix = fp[1:3], format = "cumulative", mode = "c")
  mc2 <- bayes_beta_c(~ x + y, data = DC, fix = fp[1:3], format = "cumulative")
  mcc <- bayes(~ x + y, data = DC, fix = fp[1:3], format = "count", mode = "c")
...
test_that("Predicted Values for 1 Alternative", {
  ...
  m3 <- bayes_beta_c(~ x, D, fix = fp[c(1,2,5)])
...
test_that("Learning rate 'delta'", {
  ...
  M <- bayes(~ x + y, data = D, fix = list(delta=0, x=1.7, y=0.3))
