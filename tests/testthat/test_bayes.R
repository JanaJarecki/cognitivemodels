...
test_that('Predicted Values for 1 row, 2 alternatives', {
  ...
  fp <- list(delta = 1, x=1, y=1, z=1, `I(1 - x)`=1, `I(1 - y)`=1, sigma=0)
  m1 <- bayes(~ x + y, data = D[1,],  fix = fp[c(1:3,7)], mode = "c")
  m2 <- bayes(~ x + y, D[1,], fix=list(delta=1, priorpar=c(1,1), sigma=0), mode = "c")
...
test_that("Predicted Values for 2 Stimuli a 1 Alternative", {
  ...
  m2 <- bayes(~ x | y, D, fix=list(delta=1, priorpar=c(1,1,1,1), sigma=0))
...
test_that("Learning rate 'delta'", {
  ...
  DC <- as.data.frame(apply(D, 2, cumsum))
  fp <- list(delta = 1, x=1, y=1, z=1, `I(1 - x)`=1, `I(1 - y)`=1)
  M <- bayes(~ x + y, data = D, fix = list(delta=0, x=1, y=1, sigma=0))
  expect_equivalent(M$predict(), rep(0.5, 4))
  M <- bayes(~ x + y, data = D, fix = list(delta=0, x=1.7, y=0.3, sigma=0))
  expect_equivalent(M$predict(), rep(0.85, 4))
  M <- bayes(~ x + y, data = D, fix = list(delta=10, x=1, y=1, sigma=0))
